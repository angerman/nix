...
void Compiler::compileLambda(ExprLambda * e)
{
    ...
    // Env layout matches the tree-walker's callFunction:
    //   Simple: size=1 (slot 0 = arg)
    //   @{ f1, f2 }: size=1+N (slot 0 = @, 1..N = formals)
    //   { f1, f2 }: size=N (slots 0..N-1 = formals)
    uint16_t envSize = (!e->arg ? 0 : 1)
        + (formals ? static_cast<uint16_t>(formals->formals.size()) : 0);
    if (!formals) envSize = 1; // simple lambda
