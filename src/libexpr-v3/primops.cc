                             ...
                             nix::Value ** args, nix::Value & out)
{
    ...
    nix::Symbol resolvedName = ns.symbols.create(name);
    try {
        // FFI_KILL_PLAN Phase A2 / L2 short-circuit (2026-06-01):
        // when `*found` is already a Bridge thunk wrapping a TW
        // nix::Value*, return the original TW pointer directly.
        // Avoids a round-trip:  TW Value → v3 Bridge thunk → forceValue
        // inside v3ToTreeWalker → bridges back to TW.
        //
        // Same short-circuit that v3ToTreeWalker performs at its top
        // (primops.cc:~5330) — but applied here we avoid the function
        // call entirely.  Saves one v3ToTreeWalker invocation per
        // bridged-attr force whose value is itself a TW handle.
        //
        // Diagnostic counter under NIX_V3_BP2_FAST_PATH_DEBUG (no-op
        // when unset) is left out per [[falsification-rule]]: the win
        // is correctness-equivalent + cheaper; no gate, no carcass.
        if (found->tag() == Tag::Thunk && found->payload.thunk
            && found->payload.thunk->state == ThunkState::Bridge
            && found->payload.thunk->bridgeSrc)
        {
            ++allocStats().v3ToTwBySite[11];  // still attributable to BP2 path
            nix::Value * orig = static_cast<nix::Value *>(
                found->payload.thunk->bridgeSrc);
            // Caller (TW) will forceValue on `out` per WHNF protocol.
            // Mirror v3ToTreeWalker's #483 part 5 decision to NOT
            // eager-force here.
            if (orig) out = *orig; else out.mkNull();
            return;
        }
...
// FFI_KILL_PLAN Phase A1 (2026-06-01): primV3ForceListElem retired.
// Inventory confirmed 0 calls on all measured workloads (hello/HNE/M5/
// firefox/gcc/bash/python3/synthetic-IFD).  Tag::List → TW now always
// eager-bridges (see v3ToTreeWalker case Tag::List below); the lazy-
// list construction path that allocated `__v3_force_list_elem`
// PrimOpApp chains was deleted alongside the primop body.
//
// What this kills: "BP3 has hidden callers" — KILLED.
// What this preserves: BP1 (call_bridge_1) + BP2 (force_attr) are
// still load-bearing on flake-fetch paths.
                                    ...
                                    std::unordered_map<const void *, nix::Value *> & seen)
{
    ...
    case Tag::List: {
        // FFI_KILL_PLAN Phase A1 (2026-06-01): always eager-bridge.
        // The prior n > kEagerListMax branch constructed a lazy chain
        // of `__v3_force_list_elem` PrimOpApp Values; inventory
        // confirmed 0 calls to BP3 on all measured workloads, so the
        // lazy-bridge construction was pure overhead (per-element
        // vIdx + vApp Boehm allocations).  Retiring saves those
        // allocations on the rare large-list-to-TW path.
        //
        // Risk: a workload with HUGE lists (millions of entries)
        // crossing to TW would eager-convert all elements.  Per
        // inventory measurement: 0 calls on hello/HNE/M5/firefox/
        // gcc/bash/python3/synthetic-IFD — scenario doesn't fire.
        // NIX_V3_EAGER_BRIDGE_MAX still applies to Tag::Attrs below.
        auto * lv = v.payload.list;
