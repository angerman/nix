...
void primReadFile(EvalState & state, Value * args, Value & out)
{
    ...
    if (state.nixEvalState) {
        ...
        ss << f.rdbuf();
        content = ss.str();
        // No TW EvalState (v3-eval standalone): no store to hash against.
...
// end-of-eval DIAG clears (bridges + import-cache results) to a single
// public entry point for the `nix eval` CLI to call AFTER rendering
// completes.
//
