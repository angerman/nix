...
#include "v3/vm_internal.hh"      // extracted diagnostic/trace helpers (vm_debug.cc)
...
// kMaxIndirectionChase moved to v3/vm_internal.hh (shared: coerceToString's
// Slot-chase in vm_values.cc + the force/chase loops here).  kMaxCallDepth stays.
...
// g_sharedWbDetect moved to vm_values.cc (external; decl in v3/vm_internal.hh)
// — shared with mergeBindings' WS-A producer sites + the reporter below.
static const bool g_sharedWbAbort  = std::getenv("V3_DBG_SHARED_WB_ABORT") != nullptr;

// chainChildCount() (WS-A per-chain-parent counter) moved to vm_values.cc
// (external; decl in v3/vm_internal.hh; iterated by the reporter below).
...
// V3_DBG_FORCE_SITE / V3_DBG_FORCE_INSIDE_X force tracers (dbgLogForceSite /
// dbgLogForceInsideX) + their NAMESPACE-scope T2 gates (g_dbgForceSite /
// g_dbgForceInsideX) were extracted to vm_debug.cc — see v3/vm_internal.hh.
...
// dbgLogForceInsideX (V3_DBG_FORCE_INSIDE_X) + dbgLogForceSite
// (V3_DBG_FORCE_SITE) were extracted to vm_debug.cc — declared in
// v3/vm_internal.hh.  Cold trace helpers; call sites below are unchanged.
...
// valueRepr — declared in v3/vm_internal.hh, defined in vm_values.cc.
...
// valueRepr (TW ValuePrinter mirror) + coerceToString's doc moved to
// vm_values.cc (step 4 of the vm.cc split; decls in v3/vm_internal.hh).
// throwNonBooleanCondition below stays file-local and calls external valueRepr.
...
// v3/vm_internal.hh so the OP_ATTRS_UPDATE / extends / compose call sites below
// still name MergeBindingsSite::… unchanged.
...
// Captured-withs singleton interning (the 4096-bucket 1-element capturedWiths
// cache: internOrAllocSingletonCapWiths / snapshotCurrentWiths / clearCapWiths-
// Cache / refreshCapWithsCacheAfterScavenge / get*CapWiths* stats) was extracted
// to vm_interning.cc — declared in v3/vm_internal.hh (+ v3/vm.hh for the GC /
// stats wrappers).  pushCapturedWiths (below) stays inline here (hot call-path
// loop, deliberately NOT extracted).  Call sites are unchanged.
...
// NIX_TRACE_EVAL helpers v3ValueTypeName / v3ThunkTracePos (used by
// OP_FORCE / OP_RETURN and forceValue to emit F / W events) were extracted
// to vm_debug.cc — declared in v3/vm_internal.hh.  hotForceCheck stays here:
// it is an unconditional hot-force-path gate whose inlined nullptr early-out
// is load-bearing.
...
namespace nix::v3 {
    // capWiths external-linkage wrappers (getCapWithsHits / Misses / Evicts +
    // refreshCapWithsCacheAfterScavenge) were extracted to vm_interning.cc —
    // declared in v3/vm.hh.  Called from run.cc (NIX_VM_STATS) + gc.cc
    // (post-scavenge rekey).
...
// ---------------------------------------------------------------------------
// LEVER-1 applied-import RESULT cache — the DECISION cluster (policy /
// key-building / probe / shadow-validation) was extracted to vm_applied_cache.cc
// (step 3 of the vm.cc split).  Moved: appliedCacheOn / appliedCacheShadowMode /
// appliedCacheTryKey / appliedShadowCompare / appliedCacheProbeObserve (the five
// cross-TU entry points, now declared in v3/vm_internal.hh) plus the file-local
// helpers AppliedCacheProbeStats / appliedCacheProbeStats / kAppliedKeyBudget /
// appliedProbeBoundedKey / appliedKeyPrecheck / appliedShadowCompareOne.  The
// cache STORAGE layer (appliedCacheLookup / Insert / LookupPeek / Note* /
// StatsDump / RecordImportResult / IsImportResultDesc) already lives in
// primops.cc (v3/primop.hh) — unchanged.  Call sites below (OP_CALL /
// OP_TAIL_CALL / callClosure apply paths + OP_RETURN shadow-compare/insert) are
// unchanged.
// ---------------------------------------------------------------------------
...
#pragma GCC diagnostic ignored "-Wswitch-enum"
        ...
        case OP_CALL: {
            ...
            // AppliedCacheProbeStats block in vm_applied_cache.cc for the design
...
// v3ValueTypeName / v3ThunkTracePos: extracted to vm_debug.cc (declared in
// v3/vm_internal.hh) so both dispatchLoop's OP_FORCE / OP_RETURN and
// forceValue below can call them.
...
// LEVER-1 applied-import cache PROBE cumulative-counter dump
// (dumpAppliedCacheProbeStats, called from run.cc at end-of-root-eval) was
// extracted to vm_applied_cache.cc alongside the probe counters it reads.
...
Value callClosure(VMState & vm, Value fun, Value arg)
{
    ...
    // OP_CALL hook; see vm_applied_cache.cc's AppliedCacheProbeStats for design.
