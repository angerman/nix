...
| `repro-nylon-derivation-hybrid.nix` | **KNOWN-FAIL** synthetic minimal repro: `lib.evalModules` with `lib.mkIf (lib.filter cfg)` pattern triggers v3-only infinite-recursion in `_module.freeformType`. Bisection pinpointed bytecode derivation-hybrid INSTALL (not call!) as the trigger. Workaround: `NIX_V3_NO_BC_DERIVATION_HYBRID=1`. See `project_bc_deriv_hybrid_module_cycle.md` for full analysis. | (2026-05-20, deferred) | Currently fails under v3-direct; matches TW with workaround gate |
