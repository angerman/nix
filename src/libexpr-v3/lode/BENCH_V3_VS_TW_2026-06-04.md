...
2. **`fold-add-1M` is v3's worst case at 17.5×** — primop-driven iteration
   (`foldl'`/`genList` calling a v3 lambda 1M×). The per-lambda-call overhead
   (OP_CALL frame setup + return) dominates; TW's `foldl'` is a tight C loop.
   **This is the highest-leverage eval-CPU target** (hot map/fold/filter loops
   are pervasive in nixpkgs).
