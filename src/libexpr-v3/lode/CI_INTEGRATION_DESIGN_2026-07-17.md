...
**Feasibility for the zw3rk jobsets specifically is GOOD:** `runV3DirectEval` handles exactly the `--expr`/`--file` + attrPath (`InstallableAttrPath`) shape, and rejects flake installables ("Phase 2", `eval.cc:80-86`). The zw3rk jobsets are **legacy `.nix`-expression** jobsets (`release.nix` / `ci.nix` via `importPinned`), i.e. the supported shape — **not** flakes. *One external fact to confirm against Hydra's jobset config: that the jobset type is legacy-expr, not flake.* If any target jobset is flake-based, the flake front-end is additional (Phase-2-class) work.
