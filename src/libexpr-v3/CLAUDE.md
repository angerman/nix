...
0. **The generational nursery + Phase-D write barriers + gen-major collection are SHIPPED and DEFAULT-ON** (flip `e863f127d`; opt-out RETIRED `3c17abb08`). `barrier.cc` hardcodes `g_phaseDActive = true`, the nursery/scavenge/gen-major gates are hard constants, and `NIX_V3_NURSERY` / `_SCAVENGE` / `GEN_MAJOR` are now NO-OPS. **You MUST assume nursery + moving-GC semantics in all v3 code**: any tenured object holding a nursery payload needs a Phase-D barrier + a scavenger walker, or it's a missed-root UAF (the PhD-6 class). The legacy per-op major GC (`alloc.hh g_majorGcEnabled`) stays hard-`false` — re-enabling it alongside the always-on nursery is the M-3 UAF trap; the `NIX_V3_EVAC` mid-eval evacuation experiment that required it was REMOVED in the 2026-08-05 cleanup (`7cce46a15`). Boehm is still the underlying page allocator, but the generational layer above it is active. Environment-sharing (`Closure/Thunk` upvalues in a shared `Env`) and env *interning* were both REMOVED in that same cleanup (batch 5, STEP 2 `6b874983e` + STEP 3 `9ef64f144`): the shared-`Env` path was proven interning-fed and always-null by default (`shareAfter()` returned `UINT32_MAX`), so the `upvalEnv` field, the `THUNK_ENV_SHARED` flag, `allocThunkSuspendedShared`, the intern table, and the `NIX_V3_ENV_INTERN` / `_ENV_SHARE_AFTER` / `_NO_ENV_SHARING` / `_ENV_SHARING` gates are all gone. **Inline-FAM upvalue capture is now the sole, unconditional path** (Closure header shrank 32→24 B). The generic `Env` scaffolding referenced by earlier revisions of this note — `allocEnv`, `Scavenger::walkEnv`, `envPostConstructBarrier`, `GK_ENV`, `DirtyKind::Env`, `CallFrame::defEnv`, and `CellType::Env` — was **REMOVED** in `6748ea04c` ("retire dead env-sharing scaffolding"): it had never been fed (no code ever created a shared `Env`), so it was dead infra, not future-proofing. **There is no `Env`-chain producer; inline-FAM capture is the whole story.** Do NOT chase those names as live symbols — only two comment *tombstones* remain (a `GK_ENV` mention in `gc.cc`, a `defEnv` note in `serialize.hh`). `CellType` now has 8 values (0-7, no `Env`). Any stray `NIX_V3_NO_ENV_SHARING` toggle left in a bench/script is inert (the gate is gone). Stress every force-path / GC change with the full `--brute` (see the pre-merge gate above). Background: LESSONS §1.6, `NURSERY_PHASE_D_DESIGN_2026-05-18.md`, the project memory's Phase-D / FP-4 / flip entries.
